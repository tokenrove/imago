...
(defun interpolate-pixel-cubic (image x y bytepositions)
        ...
        (labels ((compute-neighborhood (real-y real-x neighborhood)
                 ...
                 (cubic (x p0 p1 p2 p3)
                   ...
                   (let* ((step1 (* x (+ (* 3f0 (- p1 p2)) p3 (- p0))))
                          ...
                          (step3 (* x (+ p2 (- p0) step2))))
                     (+ p1 (* 0.5f0 step3))))
                 (bicubic (x y pixels bytepos)
                   (declare (type single-float x y))
                   (declare (type (simple-array (unsigned-byte 32) (16)) pixels))
                   (macrolet ((px (n) `(float (ldb (byte 8 bytepos) (aref pixels ,n)) 1f0)))
            ...
            (multiple-value-bind (real-y interp-y) (truncate y)
              (let ((neighborhood (make-array 16 :element-type '(unsigned-byte 32))))
                ...
                (let ((result 0))
                  ...
                  (dolist (bytepos bytepositions result)
                    (declare (type (unsigned-byte 8) bytepos))
                    (let* ((interpolated (bicubic interp-y interp-x neighborhood bytepos))
                           (clamped (clamp (round interpolated) 0 255))
                           (shifted (ash clamped bytepos)))
                      (declare (type (unsigned-byte 8) clamped)
                               (type (integer 0 32) bytepos))
                      (incf result shifted))))))))))))

(defmethod interpolate-pixel ((image rgb-image) x y (method (eql :bicubic)))
  (interpolate-pixel-cubic image x y '(0 8 16 24)))

(defmethod interpolate-pixel ((image grayscale-image) x y (method (eql :bicubic)))
  (interpolate-pixel-cubic image x y '(0 8)))
