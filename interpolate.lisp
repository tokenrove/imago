...
(defmethod interpolator ((image image) (method (eql :nearest-neighbor)))
  ...
  (with-image-definition (image width height pixels)
    (declare (type image-dimension width height)
...
(declaim (inline linear-interpolator))
(defun linear-interpolator (pixels constructor accessors)
...
(declaim (inline cubic-interpolator))
(defun cubic-interpolator (image-pixels constructor accessors)
  (destructuring-bind (height width)
      (array-dimensions image-pixels)
    (declare (type image-dimension height width))
    (lambda (x y)
      (declare (type (single-float 0f0 1f0) x y))
      (let ((x (- (* x width) 0.5f0))
            (y (- (* y height) 0.5f0)))
